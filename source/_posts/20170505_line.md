...
| 存储方式        | 优点           | 缺点  |
| ------------- |:-------------:| -----:|
| 顺序存储    | 1.无需为表中逻辑关系增加额外的存储空间(空间是紧挨着的) 2.读取元素速度快 | 1.插入,删除操作复杂速度慢  2.易造成空间碎片化 |
| 链式存储 | 1.插入删除操作相对快速     |  1.查询操作相对较慢  |
...
```aidl
            ...
            Status ListInsert(SqList *L, int i, ElemType *e)// i是删除的位置 *e是删除的元素
            {
                int k;
...
```aidl
           // 1. 获取list中的某个元素
            ...
            Status ListInsert(LinkList *L, int i, ElemType *e)// i是删除的位置 *e是删除的元素
            {
                int j;
                LinkList p, q;
