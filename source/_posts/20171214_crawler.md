...
description: 本人第一次学习python 根据下面的链接里的教程学习爬虫

toc: true
   
---

听说爬虫特别有成就感，非常有利于学习and娱乐，培养学习的兴趣，so begin

*我的使用Anaconda管理包和环境（py3.6）*
...
def getList():
    resp = request.request.urlopen('https://movie.douban.com/nowplaying/beijing/')  #获取url下的影片列表;python2.x下使用urllib.urlopen()
...
def getComments(nowplaying_movie_list, num):
    ...
    resp = request.request.urlopen(requrl)
